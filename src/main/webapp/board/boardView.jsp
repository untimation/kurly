...
<body>
	...
	<div id="wrap" align="center">
		...
		<h1 class="post">게시글 상세보기</h1>
		<hr class="line">

		<table class="table">
			<tr class="table_title ta">
				<th class="table_title_ti ti">제목</th>
				<td colspan="3" class="title_text tx">${board.title}</td>
			</tr>
			<tr class="table_name ta">
				<th class="table_name_ti ti">작성자</th>
				<td class="name_text tx">${board.username}</td>
			</tr>
			<tr class="table_date ta">
				<th class="table_date_ti ti">작성일</th>
				<td class="date_text tx"><fmt:formatDate
						value="${board.writedate}" /></td>
			</tr>
			<tr class="table_cont">
				<%-- 내용 --%>
				<td colspan="3" class="cont_text">${board.content}</td>
			</tr>
		</table>
		<div class="btn_box">
			<input type="button" value="수정" class="up_btn btn"
				onclick="open_win('BoardServlet?command=board_check_pass_form&num=${board.num}','update')">
			<input type="button" value="삭제" class="del_btn btn"
				onclick="open_win('BoardServlet?command=board_check_pass_form&num=${board.num}','delete')">
			<input type="button" value="목록" class="list_btn btn"
				onclick="location.href='BoardServlet?command=board_list'"> 
			<input type="button" value="등록하기" class="wri_btn btn"
